 ...
 * of MetricsMutables, making writing metrics source easier.
 */
public class MetricsRegistry {
	private final Map<String, MutableMetric> metricsMap = Maps
			.newLinkedHashMap();
	 ...
	 * @return a new counter object
	 */
	public synchronized MutableCounterLong newCounter(MetricsInfo info,
			long iVal) {
		checkMetricName(info.name());
	...
	public synchronized MutableQuantiles newQuantiles(String name, String desc,
		...
		MutableQuantiles ret = new MutableQuantiles(name, desc, sampleName,
				valueName, interval);
	...
	public synchronized MutableStat newStat(String name, String desc,
		...
		checkMetricName(name);
		MutableStat ret = new MutableStat(name, desc, sampleName, valueName,
				extended);
		metricsMap.put(name, ret);
		return ret;
	}
	 ...
	 */
	public MutableStat newStat(String name, String desc, String sampleName,
			String valueName) {
...
//	public MetricMutableRate newMutableRate(String name) {
//		return newMutableRate(name, false);
//	}
//
//	public MetricMutableRate newMutableRate(String name, boolean returnExisting) {
//
//		if (returnExisting) {
//			MutableMetric rate = metricsMap.get(name);
//			if (rate != null) {
//				if (rate instanceof MetricMutableRate)
//					return (MetricMutableRate) rate;
//				throw new MetricsException("Unexpected metrics type "
//						+ rate.getClass() + " for " + name);
//			}
//		}
//		checkMetricName(name);
//		MetricMutableRate ret = new MetricMutableRate(name);
//		metricsMap.put(name, ret);
//		return ret;
//	}
	...
	public synchronized void add(String name, long value) {
		...
		if (m != null) {
			if (m instanceof MutableStat) {
				...
				throw new MetricsException("Unsupported add(value) for metric "
						+ name);
			}
		} else {
	 ...
	 */
	public synchronized MetricsRegistry tag(MetricsInfo info, String value,
			boolean override) {
		if (!override)
	...
	public String toString() {
		return Objects.toStringHelper(this).add("info", metricsInfo)
